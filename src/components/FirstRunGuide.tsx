...
const steps = [
  ...
  { title: 'Creating as a host', body: 'Tap Create, choose a Quiz event or Giveaway event, add the reward budget, then share the event link or QR code with your audience.', icon: <Gift className="w-5 h-5 text-[#121417]" /> },
  { title: 'Review and pay entries', body: 'Open Creator Hub → Wallets & Payouts. Quiz claims and giveaway wallet submissions appear there. Close a giveaway, create payouts for all eligible entries, then approve each NIM transfer in Nimiq Pay. EventQuest queues them; it does not send one atomic batch transaction.', icon: <Wallet className="w-5 h-5 text-white" /> },
]
